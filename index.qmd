...
pour laisser le code recréer les chemins absolus :
...
<br/>This means data files must be separate from code files. And that output files are separated too.
...
<br/>
Le code (les méthodes) est factorisé en morceaux réutilisables (des functions),
elle sont appellées par les scripts d'analyses :
<br/>
```
    .
...
Attention le répertoire `R` ne doit contenir que des fichiers `.R` qui ne contienne que des définitions de fonctions !

Tout code présent dans `R` sera executé lors d'un appel aux fonctions `devtools::load_all()` ou `targets::tar_source()`, bien utiles par ailleurs !
:::
...
Il existe des modèles d'organisations plus ou moins complexes, on peut aussi adapter la compléxité du compendium à celle du projet.
...
- Utilisez un script "maître" (`make.R`) qui execute les étapes dans l'ordre (c'est un des SEULS scripts R à la racine !)
...
::: {.incremental}
- Il est compliqué, lorsque l'analyse n'est pas linéaire ou longue, de se souvenir de toutes les dépendances entre scripts, données et résultats ...
...
Utilisez le fichier [DESCRIPTION]{.blue} et le package [renv]{.blue} pour les packages !
...
## Research  Compendium {auto-animate=true}
...
```
    ...
    ├── [analyses] (scripts)
    ├── [outputs] (résultats)
    ├── [syntheses] (article, présentation)
    ├── [docs] (biblio)
    ├── my_project.Rproj
    ├── README.md
    ├── DESCRIPTION
...
![](assets/img/1908-iceberg-lineal.gif){width="300" fig-align="center"}
