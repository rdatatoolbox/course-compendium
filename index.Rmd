...
- Par ex., numérotez les scripts, placez-y des entêtes
...
Utilisez un script **principal** (par ex. `make.R`) qui exécute les étapes dans l'ordre 
(c'est le SEUL script R à la racine !)
...
Cela peut poser des problèmes lors de la compilation des `.Rmd` situés à 
l'intérieur de votre arborescence. `-->` Utilisez le package [`{here}`](https://here.r-lib.org)
...
- L'utilisation d'un projet RStudio (`.Rproj`) permet de se retrouver à la racine du projet 
à son ouverture.
...
.center[Par ex. le script `R/01_ingest_data.R` écrit des résultats du type `res/01_Zzz9Zzz9.truc`]
...
- Séparez aussi le(s) documents de synthèse (papier, présentation, ...)
...
- Séparez aussi les ressources utiles (biblio, etc ...)
...
- Il existe `Zenodo` (https://zenodo.org/) : il a été créé par OpenAIRE et le CERN 
pour fournir aux chercheurs un lieu pour déposer des ensembles de données. 
Il a été lancé en 2013, permettant à des chercheurs de télécharger des fichiers 
jusqu'à 50 Go.
<br />
<br />
<br />
...
- https://github.com/benmarwick/rrtools (structure de package + intégration de Docker !!)
...
- On est souvent amener à refaire tourner l'ensemble des analyses...
